 ...
 *  passed verbatim to the provider. `remoteUrl` is the only forge-relevant
 *  field beyond repo+branch; providers that need it (Forgejo, for host
 *  detection + token lookup) read it; providers that don't (GitHub, which
 *  resolves via git remote tracking from the repo root) ignore it. */
